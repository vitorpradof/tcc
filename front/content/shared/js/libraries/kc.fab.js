        ...
        base.init = function(){
            ...
            if (base.links.length > 0){
                ...
                for (var i = 0; i < base.links.length; i++) {
                    ...
                    if (base.links[i].function !== undefined) {
                        sub_fab_btns_dom += "<div><button data-link-title='"+base.links[i].title+"' onclick='"+base.links[i].function+"' class='sub_fab_btn' style='"+bg_color_style+"'><span style='"+color_style+"'>"+base.links[i].icon+"</span></button></div>";
                    } else {
            ...
            sub_fab_btns.find('.sub_fab_btn').on('mousedown', function(e){
                let atributo = ($(this).attr("onclick"));
                if (typeof atributo === typeof undefined && atributo === false) {
                    if ($(this).attr('data-link-href').length > 0){
                        if ($(this).attr('data-link-target')){
                            window.open($(this).attr('data-link-href'), $(this).attr('data-link-target'));
                        }else{
